...
섹터 등락률 + 뉴스 키워드 기반으로 한국 종목 점수를 산출한다.
- 섹터 점수: 60% 가중치
- 뉴스 점수: 40% 가중치
- 최소 1.0점 이상만 추천
...
from .mappings import KEYWORD_MAP, SECTOR_ETF_MAP, KrEtfMapping, KrStock
...
def calc_sector_scores(
    ...
    for etf, pct_change in sector_changes.items():
        ...
        _accumulate_scores(raw, info, stocks, pct_change)
...
def score_stocks(
    ...
    # 종목 정보 통합 (섹터 정보 우선, 없으면 뉴스 정보)
    all_codes = set(sector_scores) | set(news_scores)
    stock_info: dict[str, KrStock] = {}
    for code in all_codes:
        stock_info[code] = sector_info.get(code) or news_info.get(code)  # type: ignore[assignment]
    ...
    for code in all_codes:
        ...
        total = round(s_score * SECTOR_WEIGHT + n_score * NEWS_WEIGHT, 2)
